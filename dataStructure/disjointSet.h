...
template<typename T>
disjointSet<T>::disjointSet(int len)
{
...
template<typename T>
int disjointSet<T>::intialize(int len)
{
	setMap.clear();
	if (setNumP)
		free(setNumP);
...
template<typename T>
int disjointSet<T>::insert(T element)
{
	if (nextIndex == setLen)
		return -1;
...
template<typename T>
int disjointSet<T>::getFa(T element)
{
	if (setMap.find(element) == setMap.end())
		return -1;
...
template<typename T>
int disjointSet<T>::getSetNum(T Ele)
{
	int id = getFa(Ele);
	if (id == -1)
		return 0;
...
template<typename T>
int disjointSet<T>::merge(T ele1, T ele2)
{
	int id1 = getFa(ele1);
	int id2 = getFa(ele2);
